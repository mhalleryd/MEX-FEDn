...
from scaleoututil.helpers.helperbase import HelperBase
...
class Helper(HelperBase):
    ...
    def save(self, data_dict, path=None, file_type="npz"):
        ...
        ScaleoutLogger().info("SPLIT LEARNING HELPER: Saving data to {}".format(path))
    ...
    def load(self, path):
        ...
        try:
            data = np.load(path)
            ScaleoutLogger().info("SPLIT LEARNING HELPER: loaded data from {}".format(path))
            result_dict = {k: data[k] for k in data.files}
            return result_dict
        except Exception as e:
            ScaleoutLogger().error(f"Error in splitlearninghelper: loading data from {path}: {str(e)}")
            raise
