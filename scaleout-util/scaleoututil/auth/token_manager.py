...
class TokenManager:
    ...
    def _should_refresh_token(self) -> bool:
        ...
        if should_refresh:
            ScaleoutLogger().info(f"Token needs refresh. Expires in {time_until_expiry:.0f}s (buffer: {TOKEN_REFRESH_BUFFER_SECONDS}s)")
    ...
    def _extract_expiration(self, token: str, fallback_expires_in: Optional[int] = None) -> datetime:
        ...
        try:
            ...
            if exp_timestamp:
                ...
                ScaleoutLogger().info(f"Extracted expiration from JWT: {expiration} (in {time_until_exp:.0f} seconds)")
                return expiration
        except Exception as e:
        ...
        ScaleoutLogger().info(f"Using fallback expiration: {fallback} seconds from now ({expiration})")
        return expiration
    ...
    def _perform_token_refresh(self) -> None:
        ...
        ScaleoutLogger().info("Refreshing access token...")
        ...
        try:
            ...
            if response.status_code == HTTP_STATUS_UNAUTHORIZED:
                ...
                ScaleoutLogger().info("The refresh token may have been revoked or expired. You may need to log in again to obtain a new refresh token.")
    ...
    def update_tokens(self, access_token: str, refresh_token: Optional[str] = None, expires_in: Optional[int] = None) -> None:
        ...
        with self._lock:
            ...
            ScaleoutLogger().info("Tokens updated manually")
