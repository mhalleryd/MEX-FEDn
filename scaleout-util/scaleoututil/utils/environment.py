...
class PythonEnv:
    ...
    def path(self) -> Path:
        ...
        return Path(self._base_path).joinpath(self._name)
    ...
    def remove_scaleoutdependency(self):
        ...
        self.dependencies = [dep for dep in self.dependencies if dep != "fedn" and dep != "scaleout"]
        self.build_dependencies = [dep for dep in self.build_dependencies if dep != "fedn" and dep != "scaleout"]
    ...
    def install_into_current_env(self, capture_output=False):
        ...
        for deps in filter(None, [self.build_dependencies, self.dependencies]):
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                cmd = [sys.executable, "-m", "pip", "install", "-r", tmp_req_file]
        ...
        Path(self.path).joinpath(_PYTHON_ENV_METADATA_FILE_NAME).write_text(f"{self.get_sha()}\n{self.python}\n{build_deps}\n{deps}")
    ...
    def create_virtualenv(self, capture_output=False):
        ...
        with remove_on_error(
            ...
            ScaleoutLogger().info("Installing dependencies")
            for deps in filter(None, [self.build_dependencies, self.dependencies]):
                with tempfile.TemporaryDirectory() as tmpdir:
                    tmp_req_file = f"requirements.{uuid.uuid4().hex}.txt"
                    Path(tmpdir).joinpath(tmp_req_file).write_text("\n".join(deps))
