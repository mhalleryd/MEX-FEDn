...
class APIClient:
    ...
    def __init__(
        ...
        if _protocol is not None and secure is not None:
            print("Both protocol in host and secure parameter provided. Using protocol from host.")
        ...
        # Try to load token from cache first (for api-client, we use a fixed cache ID)
        token_cache = TokenCache(cache_id="api-client", cache_dir=os.environ.get("SCALEOUT_TOKEN_CACHE_DIR", None))
